...
import type { PlatformSmtpResponse } from '@/api/modules/platform'
...
/**
 * SMTP 表单校验：已配置时密码可留空（= 不修改），首次配置必填。
...
function buildSmtpSchema(configured: boolean) {
  return z.object({
    ...
    password: configured
      ? z.string()
      : z.string().min(1, '首次配置需填写 SMTP 密码'),
...
function SmtpForm({ data }: { data: PlatformSmtpResponse }) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('SMTP 配置已保存', { description: '配置版本已更新，此前的验证结果已失效。' })
      void queryClient.invalidateQueries({ queryKey: ['platform', 'smtp'] })
    },
    onError: (error) => {
  ...
  const testMutation = useMutation({
    mutationFn: () =>
      testPlatformSmtp(testRecipient.trim() ? { recipient: testRecipient.trim() } : {}),
    onSuccess: (result) => {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            平台 SMTP 用于发送负责人邀请等平台级邮件；密码由服务端加密存储，永不回显。
          </CardDescription>
        </CardHeader>
        <CardContent>
          {configured ? (
            ...
            <p className="text-sm text-muted-foreground">
              尚未配置 SMTP。未配置时，依赖发信的业务（如负责人邀请）将无法执行。
            </p>
          )}
        </CardContent>
      </Card>
          ...
          <form noValidate onSubmit={onSubmit} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="smtp-host">服务器地址 *</Label>
                <Input
                  id="smtp-host"
                  spellCheck={false}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="smtp-password">密码 {configured ? '' : '*'}</Label>
                <Input
                  id="smtp-password"
                  type="password"
            ...
            <div className="space-y-2">
              ...
              {form.formState.errors.from ? (
                ...
                <p className="text-xs text-muted-foreground">
                  格式如：Rollin &lt;noreply@example.edu.cn&gt;
                </p>
              )}
            </div>
...
export function SmtpPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="SMTP 配置" description="平台级发信服务器，用于负责人邀请等平台邮件" />
      ...
      {smtpQuery.isPending ? (
        ...
        <SmtpForm key={smtpQuery.data.configVersion ?? 'unconfigured'} data={smtpQuery.data} />
      )}
    </div>
  )
}
